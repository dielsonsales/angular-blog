...
def skip?(path)
  ...
  parts.any? { |p| p.start_with?('.') } ||
    ...
    parts.include?('.angular') ||
    parts.include?('.vscode') ||
    parts.include?('.github') ||
    parts.include?('public') ||
    path.basename.to_s == 'generate_content.rb' ||
    path.basename.to_s == 'file_structure.md' ||
    path.basename.to_s == 'package-lock.json' ||
